...
def make_afrl_df(source_file, resolution='1D', series='Agg8.25D'):
    ...
    idx = pd.date_range(np.min([north.index[0], south.index[0]]), np.max([north.index[-1], south.index[-1]]), 
                        freq=resolution)

    north = north.resample(resolution).median().reindex(idx, fill_value=np.nan)
    north_min = north_min.resample(resolution).median().reindex(idx, fill_value=np.nan)
    north_max = north_max.resample(resolution).median().reindex(idx, fill_value=np.nan)
    
    south = south.resample(resolution).median().reindex(idx, fill_value=np.nan)
    south_min = south_min.resample(resolution).median().reindex(idx, fill_value=np.nan)
    south_max = south_max.resample(resolution).median().reindex(idx, fill_value=np.nan)

    afrl_df = pd.DataFrame(data={'north': north, 'north_min': north_min, 'north_max': north_max,
                                 'south': south, 'south_min': south_min, 'south_max': south_max})

    afrl_df['CR'] = carrington_rotation_number(afrl_df.index)

    save_name = path.join(path.dirname(path.abspath(source_file)), 'PCH_'+series+'_'+resolution+'_Bin.pkl')
