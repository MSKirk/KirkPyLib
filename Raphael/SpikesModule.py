...
class SpikesLookup:
    def __init__(self, db_filepath, data_dir, outputdir, n_co_spikes=2.):
        ...
        self.n_co_spikes = 2
        # Get the dataframe "Path" containing the list of paths of the fits files, and set index to the GroupNumber Series
        paths_df = self.spikes_db.get('Path').set_index(self.spikes_db.get('GroupNumber'))
        # Convert group number and paths to numpy arrays for faster lookup than pandas dataframe queries (x1000 speed-up, crazy...).
        self.npgroups = paths_df.Path.index.values
        self.nppaths = paths_df.Path.values
        # Filter the unique values of groups (ugroups), get associated indices (uinds) and counts for each group (ugroupc)
...
def create_good_spikes(good_1d_coords, spikes):
    ...
    for i in range(len(spikes)):
        #good_spikes_coord = np.intersect1d(spikes[0][0,:], good_1d_coord)
