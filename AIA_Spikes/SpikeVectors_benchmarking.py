...
def io_benchmark(directory):
    ...
    ffiles = [f for f in os.listdir(dir) if os.path.isfile(os.path.join(dir, f))]
    times = np.array(len(ffiles))

    for ii, f in enumerate(ffiles):
        if f.endswith('.fits'):
            startTime = datetime.now()
            sp_image = sv.spikes_to_image(dir+f)
            hdu = sv.image_to_spikes(sp_image)
            hdu.writeto(dir+f)
            times[ii] = datetime.now() - startTime
